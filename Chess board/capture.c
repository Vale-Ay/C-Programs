...
CaptureTimer() {
    ...
    if(turn == -1){ // added area for capturing the time for black player
        time_t blackEnd;
        blackEnd = time(NULL);
        blackLimit = blackLimit - ((blackEnd - blackStartCounter) / 60);
        if(blackLimit <= 0){
    ...
    if(turn == 1){ //added area for capturing the time for white player
        time_t whiteEnd;
        whiteEnd = time(NULL);
        whiteLimit = whiteLimit - ((whiteEnd - whiteStartCounter) / 60);
        if(whiteLimit <= 0){
            printf("Looks like your time is up! Quit the game and try again!");
            return;
        }
